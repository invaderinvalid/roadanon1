...
Stages (same as pipeline.py, YOLO-only — no autoencoder):
...
def run_benchmark(video_path, cfg, report_path):
    ...
    # Modules (same as pipeline.py — no autoencoder)
    ...
    while True:
        ...
        # ── Stage 5: Assign YOLO labels to new tracks ──
        for t in new_tracks:
    ...
    print(f"    YOLO calls: {yolo_calls} | Known: {known_total} | "
          f"Unknown: {unknown_total} | Motion skipped: {motion_skipped}")

    # Per-stage breakdown
    print(f"    Per-stage avg (ms):")
    for s in ['motion', 'yolo', 'tracker']:
...
def main():
    ...
    print(f"  Skip:       {base_cfg.skip_frames}")
    print(f"  YOLO conf:  {base_cfg.yolo_conf}")
    ...
    # Summary table
    print(f"\n{'='*75}")
    print(f"  BENCHMARK SUMMARY (YOLO-only — no autoencoder)")
    print(f"{'='*75}")
    print(f"  {'Video':<25} {'YOLO':<8} {'FPS':>6} {'Total':>7} "
          f"{'YOLO':>6} {'Trk':>5} {'YOcall':>6}")
    print(f"  {'-'*25} {'-'*8} {'-'*6} {'-'*7} "
          f"{'-'*6} {'-'*5} {'-'*6}")
    for s in all_summaries:
        print(f"  {s['video']:<25} {s['yolo_backend']:<8} "
              f"{s['avg_fps']:>6.1f} {s['avg_total_ms']:>6.1f}ms "
              f"{s['avg_yolo_ms']:>5.1f}ms "
